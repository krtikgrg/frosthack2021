...
class selectpg extends Component {
    constructor() {
        super();
        this.state = {
            pg : []
        ...
        await axios.post("/eventManager/getProvider", info).then(async res => {
            ...
            await axios.post("/eventManager/addProvider/all",info).then(async res => {
                    for(var i=0;i<lvp.length;i++){
                        if(res.data.pselected == 1 && lvp[i]._id == res.data.pid){
                            lvp[i].hired = 1
                        }
                        else{
                            lvp[i].hired = 0
    ...
    async onClick(e,index){
        e.preventDefault();
        var VP = this.state.pg;
        const info = {
            id: VP[index]._id,
            email: this.props.auth.user.email
        }
        await axios.post("/eventManager/addProvider/p", info).then(async res => {
    ...
    render() {
        ...
        return (
            <div style={{ display: "flex", flexDirection: "row" }}>
                ...
                <div style={{ width: "84%" }}>
                    <h3 className="center title" style={{marginBottom:"50px"}}>Lets Hire a Photographer for your next event!</h3>
