...
class selectcs extends Component {
        ...
        await axios.post("/eventManager/getProvider", info).then(async res => {
            ...
            await axios.post("/eventManager/addProvider/all",info).then(async res => {
                    for(var i=0;i<lvp.length;i++){
                        if(res.data.cselected == 1 && lvp[i]._id == res.data.cid){
                            lvp[i].hired = 1
                        }
                        else{
                            lvp[i].hired = 0
    ...
    async onClick(e,index){
        e.preventDefault();
        var VP = this.state.cs;
        const info = {
            id: VP[index]._id,
            email: this.props.auth.user.email
        }
        await axios.post("/eventManager/addProvider/c", info).then(async res => {
    ...
    render() {
        ...
        return (
            <div style={{ display: "flex", flexDirection: "row" }}>
                ...
                <div style={{ width: "84%" }}>
                    <h3 className="center title" style={{marginBottom:"50px"}}>Lets Hire a Catering Service Provider for your next event!</h3>
